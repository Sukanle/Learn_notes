...
* `结构化绑定`是`C++17`新增的特性，允许将一个储存多个值的对象（如`std::pair`、`std::tuple`等）的成员绑定到多个变量上。
  ...
  >  auto [i, d] = p;
  >  ```
  详见：[structured_binding](~/learn/learn_cpp/src/C++17/structured_binding.cpp)
...
* `C++17`中，`聚合体`可以使用`花括号`进行初始化。
  ...
  >  A a = {42, 3.14};
  >  ```
  详见：[aggregate](~/learn/learn_cpp/src/C++17/aggregate.cpp)
* 但`聚合体`需要额外满足以下条件：
...
* `C++17`中，`if`和`switch`语句允许在条件表达式里添加一条初始化语句。
  ...
  >  ```
  详见：[if_switch](~/learn/learn_cpp/src/C++17/if_switch.cpp)
...
* 语法：
  ...
  >  FuncType* func = &func;
  >  ```
  详见：[noexcept](~/learn/learn_cpp/src/C++17/noexcept.cpp)
...
* 提供了在编译期处理条件分歧的功能，通过这个`C++`就拥有了一个更加强大的条件分支系统。
  ...
  >  #endif
  >  ```
  详见：[if constexpr](~/learn/learn_cpp/src/C++17/if_constexpr.cpp)
...
* `C++17`起，`lambda`表达式的功能得到了扩展，包括：
  ...
  >  ```
  详见：[lambda](~/learn/learn_cpp/src/C++17/lambda.cpp)
...
* `[[fallthrough]]`用于告知编译器，`switch`语句中的`case`标签没有`break`，并且是有意为之的，如果没有`[[fallthrough]]`，编译器会发出警告。
  ...
  >  [[nodiscard]] int func();
  >  ```
  详见：[attribute](~/learn/learn_cpp/src/C++17/attribute.cpp)
* wei
#### *`属性适用范围扩展`*
...
* `__has_include`是一个预处理宏，用于检查指定的头文件是否存在。
  ...
  >  #endif
  >  ```
  详见：[has_include](~/learn/learn_cpp/src/C++17/has_include.cpp)
* 注意：`__has_include`是一个纯粹的预处理指令。 所以不能将它用作源码里的条件表达式。
...
* `C++17`起，类模板参数推导允许在模板类的构造函数中推导模板参数。
  ...
  >  A a(42); // T被推导为int
  >  ```
  详见：[class_template](~/learn/learn_cpp/src/C++17/class_template.cpp)
...
* `C++17`起，`lambda`表达式的参数可以使用`auto`进行类型推导。
  ...
  >  ```
  详见：[class_template](~/learn/learn_cpp/src/C++17/lambda.cpp)
...
* `C++17`起，`折叠表达式`是一种新的表达式，用于对模板参数包进行展开。
  ...
  >  ```
  详见：[fold_expression](~/learn/learn_cpp/src/C++17/fold_expression.cpp)
...
* `std::any`是一个类型安全的容器，可以存储任意类型的对象，包括基本类型、自定义类型、指针等。
  ...
  >  ```
  详见：[any](~/learn/learn_cpp/src/C++17/any_test.cpp)
...
* `std::optional`是管理一个*可选*的所含值，即可能有值，也可能没有值。
  ...
  >  opt = 42;
  >  ```
  详见：[optional](~/learn/learn_cpp/src/C++17/optional_test.cpp)
...
* `std::variant`类模板用于提供一个类型安全的`union`，即可以存储多个类型的对象，但只能同时存储其中的一个。
  ...
  >  var = 42;
  >  ```
  详见：[variant](~/learn/learn_cpp/src/C++17/variant_test.cpp)
* 与`union`的区别：
...
* `std::from_chars`是一个函数模板，用于将字符串转换为数字。
  ...
  > ```
  详见：[charconv](~/learn/learn_cpp/src/C++17/charconv_test.cpp)
#### *`std::to_chars`*
